...
| 模式 | 门禁复杂度 | 适用用户 | 门禁检查项 |
|------|-----------|---------|-----------|
| **新手模式** | 轻量（8问检查表） | 初学者、快速试错 | 8项核心检查 |
| **高级模式** | 完整（5步闭环） | 专业作者、长篇连载 | 完整质量门禁 |
...
**切换命令**：
- `/新手模式 开启`：切换到新手模式（8问检查表）
- `/新手模式 关闭`：切换到高级模式（完整5步闭环）
...
高级模式下，每章写完后固定执行以下步骤，按顺序逐项勾选，任何一项未通过都 ⛔ 禁止进入下一章：
...
- [ ] `/章末钩子检查` ⛔ — **专项检查章末钩子质量**。检查标准：
  1. 最后3段内是否出现具体钩子（问题/危机/反转/奖励/新目标）
  2. 钩子是否具体到一句话能描述清楚
  3. 是否属于禁止的结尾类型（平淡/模糊/道别/纯内心独白）
  
  **不通过条件**：无钩子、钩子模糊、属于禁止类型 → 门禁强制失败，必须改写结尾。
  **跳过后果**：读者在章末失去阅读动力，直接弃书，影响完读率和推荐量。
...
- **钩子类型**：[新问题 / 危机 / 反转 / 误会 / 奖励 / 新目标]
- **钩子具体性**：[具体 / 模糊]（模糊 = 无法用一句话描述清楚）
- **是否属于禁止类型**：[否 / 是（平淡/模糊/道别/纯内心独白）]

**悬念等级判定标准**：
- **强**：钩子具体、有明确威胁或利益、读者有强烈继续阅读的冲动
- **中**：钩子存在但不够具体，或威胁/利益不够明确
- **弱**：钩子模糊，读者可能失去兴趣
- **无**：没有钩子，平淡收尾

**结论为"失败"的条件**：悬念等级为"无" **或** 钩子具体性为"模糊" **或** 属于禁止类型
...
- **结论为"失败"的条件**：A/B/C 同时触发 ≥2项 **或** 存在隐性加速 **或** 章末悬念等级为"无" **或** 钩子具体性为"模糊" **或** 属于禁止结尾类型
