...
def call_llm(messages: list) -> str:
    ...
    def _raise_timeout():
        result_holder["_timeout"] = True
        # Interrupt the blocking call by raising in the thread
        import _thread
    ...
    for attempt in range(5):
        try:
            ...
            timer.start()
            try:
                result = call_text_llm(messages)
                if result_holder.get("_timeout"):
...
def split_sentences(text: str) -> list[str]:
    ...
    for block in re.split(r"\n+", raw):
        ...
        if not block:
            continue
        base_parts.extend(re.split(r"(?<=[。！？!?；;])", block))

    sentences = []
    for piece in base_parts:
        piece = re.sub(r"\s+", "", piece).strip("-—–•*# ")
        if not piece:
            continue
        subparts = re.split(r"(?<=[，,：:、])", piece) if len(piece) > 120 else [piece]
        buffer = ""
        for sub in subparts:
            sub = sub.strip()
            if not sub:
                continue
            candidate = f"{buffer}{sub}"
            if len(candidate) >= 90:
                ...
                buffer = candidate
        if buffer and 12 <= len(buffer) <= 160:
            sentences.append(buffer)

    filtered = []
    for sentence in sentences:
        if re.match(r"^[a-z]{2,}[A-Za-z0-9]*[一-鿿]", sentence):
            continue
        if re.match(r"^\d+(?:\.\d+){1,}[一-鿿]", sentence):
            continue
        if sentence.startswith(("的", "了", "和", "与", "并且", "以及", "所以", "但是")) and len(sentence) < 80:
            continue
    ...
    return [compact[i : i + 80] for i in range(0, min(len(compact), 800), 80) if len(compact[i : i + 80].strip()) >= 12]
...
def load_text_chunks(course_dir: str) -> list[dict]:
    path = os.path.join(course_dir, "text_sources", "chunks.jsonl")
    if not os.path.exists(path):
        return []
    rows = []
    with open(path, encoding="utf-8") as fp:
...
def keep_unique_texts(values: list[str], limit: int) -> list[str]:
    out = []
    seen = set()
    for value in values:
...
def representative_sentences(text: str, limit: int = 12) -> list[str]:
    candidates = []
    for idx, sentence in enumerate(split_sentences(text)):
        sentence = normalize_plain_text(sentence)
        core = re.sub(r"[^一-鿿A-Za-z0-9]+", "", sentence)
        if len(core) < 8:
            continue
        if len(core) < 18 and sentence.startswith(("的", "了", "和", "与", "并", "以及", "这个", "那个", "这里", "就是", "然后")):
            continue
        if len(core) < 18 and sentence.endswith(("的话", "等等", "之类", "来说")):
            continue
        score = min(len(core), 80) / 80
        for term in DOMAIN_TERMS:
            if term in sentence:
                score += 2
        if re.search(r"Claude|Cursor|GPT|RAG|SWE|API|PDF|SVG|GitHub|Copilot|benchmark", sentence, flags=re.I):
            score += 2
        if any(mark in sentence for mark in ("核心", "关键", "方法", "步骤", "流程", "原则", "评测", "生成", "使用", "应该", "需要", "可以")):
            score += 1.5
        candidates.append((score, idx, sentence))
    picked = sorted(sorted(candidates, reverse=True)[: max(limit * 2, 8)], key=lambda item: item[1])
    return keep_unique_texts([sentence for _, _, sentence in picked], limit)


def text_only_full_transcript(chunks: list[dict]) -> str:
    grouped: dict[str, list[str]] = {}
    for chunk in chunks:
        grouped.setdefault(chunk.get("source_ref") or chunk.get("source_path") or "Unknown", []).append(chunk.get("text", ""))
    sections = []
...
def local_text_only_course_digest(course_name: str, chunks: list[dict]) -> str:
    combined_text = "\n".join(chunk.get("text", "") for chunk in chunks)
    keywords = extract_keywords(combined_text, 18)
    topic_points = representative_sentences(combined_text, 6)
    method_points = representative_sentences(
        "\n".join(
            sentence for sentence in split_sentences(combined_text)
            if any(mark in sentence for mark in ("使用", "可以", "应该", "需要", "生成", "测试", "比较", "选择", "总结", "安装", "构建"))
        ),
        8,
    )
    quote_points = representative_sentences(combined_text, 10)

    per_source = []
    grouped: dict[str, str] = {}
    for chunk in chunks:
        source = chunk.get("source_ref") or chunk.get("source_path") or "Unknown"
        grouped[source] = (grouped.get(source, "") + "\n" + chunk.get("text", "")).strip()
    for source, text in grouped.items():
        highlights = representative_sentences(text, 2)
        if highlights:
            per_source.append(f"- {source}：{highlights[0]}")

    concept_lines = [f"- {kw}：课程文本中的高频主题词，需要回到原句理解具体语境。" for kw in keywords[:10]]
    topic_lines = [f"- {row}" for row in topic_points[:6]]
    method_lines = [f"- {row}" for row in (method_points or topic_points)[:8]]
    action_lines = [f"{idx}. {row}" for idx, row in enumerate((method_points or topic_points)[:8], start=1)]
    quote_lines = [f"- {row}" for row in quote_points[:10]]

    return f"""## 一、课程概览
《{course_name}》当前按纯文本材料蒸馏，共读取 {len(grouped)} 份文本来源、{len(chunks)} 个分块。以下内容只基于原始文本高频主题句和关键词整理，不补写原文未出现的结论。

## 二、课程体系图
{"\n".join(topic_lines) if topic_lines else "- 暂未抽取到稳定主题，需回看原文。"}

## 三、逐课精要
{"\n".join(per_source) if per_source else "- 暂无可用逐课文本。"}

## 四、跨课程主题图谱
- 高频关键词：{"、".join(keywords[:12]) if keywords else "未抽取到稳定关键词"}
{"\n".join(topic_lines[:4]) if topic_lines else ""}

## 五、关键概念词汇表
{"\n".join(concept_lines) if concept_lines else "- 暂未抽取到稳定概念词。"}

## 六、可执行行动清单
{"\n".join(action_lines) if action_lines else "1. 先回到原始文本确认可执行步骤。"}

## 七、核心金句集
{"\n".join(quote_lines) if quote_lines else "- 暂未抽取到稳定原句。"}
"""
...
def build_course_digest(transcripts: list, summaries: list, course_name: str, course_dir: str) -> dict:
    print(f"\n🧠 课程蒸馏 ...")
    text_chunks = load_text_chunks(course_dir)
    if not transcripts and text_chunks:
        digest = local_text_only_course_digest(course_name, text_chunks)
        return {
            "course_title": course_name,
            "total_lessons": 0,
            "total_duration_seconds": 0,
            "total_duration_hours": 0.0,
            "total_transcript_chars": sum(len(chunk.get("text", "")) for chunk in text_chunks),
            "lesson_summaries": [],
            "distillation_markdown": digest,
            "full_transcript": text_only_full_transcript(text_chunks),
