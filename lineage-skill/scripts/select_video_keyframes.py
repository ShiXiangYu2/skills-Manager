...
def media_name(path: Path, input_dir: Path) -> str:
    """Use BV code as directory name to avoid filesystem/ffmpeg issues with Chinese characters."""
    rel = path.relative_to(input_dir)
    parts = [path.stem, *rel.parts[:-1]]
    full = "_".join(parts)
    match = re.search(r"(BV[0-9A-Za-z]+)", full)
    return match.group(1) if match else full[:80]
...
def extract_candidates(video: Path, output_dir: Path, interval_seconds: int, width: int, force: bool) -> int:
    ...
    try:
        ...
        print(f"  ⚠️ 关键帧提取失败 ({video.name}): exit {exc.returncode}")
        if exc.stderr:
