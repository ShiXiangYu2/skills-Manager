...
LOW_SIGNAL_SUFFIXES = ("的话", "来说", "等等", "之类", "而已")
...
def looks_low_signal(value: str) -> bool:
    value = clean_value(value)
    if not value:
        return True
    core = signal_text(value)
    if len(core) < 4 and not re.search(r"[A-Za-z]{3,}", core):
        return True
    if len(core) <= 8 and value.startswith(LOW_SIGNAL_PREFIXES):
        return True
    if len(core) <= 8 and value.endswith(LOW_SIGNAL_SUFFIXES):
        return True
    if len(core) <= 8 and value.startswith(("来源", "这是", "那个是", "这个是", "是")):
        return True
    if len(core) <= 10 and any(token in value for token in ("什么样", "怎么做", "为什么")):
        return True
    if len(core) <= 10 and re.match(r"^[A-Za-z0-9.]+$", core):
        return True
    return False
